...
import useChangeDeleted from "../features/quests/useChangeDeleted";
...
function Quest({ quest, type = "general" }) {
  ...
  const changeDeleted = useChangeDeleted();

  const [deleteParams, setDeleteParams] = useState(null);
  const { isEditing } = useChangeDeleted(deleteParams?.id2, deleteParams?.date);

  const handleConfirm = () => {
    deleteQuest(quest.id);
    setDeleteParams({ id2: quest.id2, date: quest.date });
  };
